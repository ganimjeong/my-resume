// 기업 제출용 일본어 포트폴리오 PDF 생성기.
// 웹사이트 데이터(ja.ts) + 프로젝트 쇼케이스 상세(projectShowcase.ts)를 합쳐
// 인쇄에 최적화된 자체완결형 HTML 을 만든다. 이미지는 base64 로 임베드.
import { readFileSync } from 'node:fs';
import { resolve, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { jaData } from './src/data/ja.ts';
...
const { header, about, skills, experience, awards, languages } = jaData;
const showcase = projectShowcase.ja;

// ── 일본어 PDF 전용 오버라이드 (ja.ts / projectShowcase.ts 원본 데이터는 불변) ──
header.title = 'AIウェブサービス/業務自動化 開発者';
...
<head>
<meta charset="utf-8"/>
<title>${esc(header.name)} ポートフォリオ</title>
<style>
  @page { size: A4; margin: 14mm 13mm; }
  * { box-sizing: border-box; margin: 0; padding: 0; }
  html { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  body {
    font-family: "Hiragino Kaku Gothic ProN", "Hiragino Sans", "Noto Sans JP", "Yu Gothic", "Apple SD Gothic Neo", -apple-system, sans-serif;
  ...
  .proj-summary { font-size: 10.5px; color: #374151; line-height: 1.52; margin-top: 6px; }

  .block { margin-top: 7px; break-inside: avoid; }
  ...
  .gallery-2 .shot .shot-frame { height: 104px; }
  .gallery-3 .shot .shot-frame { height: 84px; }
  .shot-wide .shot-frame { height: 122px; }
...
const outPath = resolve(__dirname, 'portfolio-ja.html');
